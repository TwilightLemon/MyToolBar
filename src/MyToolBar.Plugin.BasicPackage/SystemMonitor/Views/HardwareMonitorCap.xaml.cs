...
using MyToolBar.Plugin.BasicPackage.SystemMonitor.API;
...
namespace MyToolBar.Plugin.BasicPackage.SystemMonitor.Views
{
    /// <summary>
    ...
    public partial class HardwareMonitorCap : CapsuleBase
    {
        ...
        {
            //30s内不重复通知
            if ((DateTime.Now - _lastNotifiedTime).TotalSeconds > 30)
            {
        ...
        private void Tick()
        {
            ...
            if(battery <= 20 && PowerManager.BatteryStatus == BatteryStatus.Discharging && CanSendNotification()&&!_lowBatteryNotified)
            {
                NotificationManager.Send(new Notification((string)FindResource("Notify_BatteryLow"), 
                    NotificationType.Warning, 
                    new NotificationSource(typeof(HardwareMonitorCap), 0, "Battery is low"),
                    NotificationTimeSpan.Long));
